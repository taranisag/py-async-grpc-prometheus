...
async def test_grpc_server_started_with_normal(
  ...
  target_metric = get_server_metric("grpc_server_started", grpc_stub.prom_server_port)
...
async def test_grpc_server_started_with_unary_stream(
...
async def test_grpc_server_started_with_stream_unary(
  ...
  target_metric = get_server_metric("grpc_server_started", grpc_stub.prom_server_port)
  assert len(target_metric.samples) == 1
...
async def test_grpc_server_started_with_bidi_stream(
